...
describe('StateOverlay', () => {
  it('paused overlay offers a resume button', () => {
    ...
    const { getByText } = render(wrap(<StateOverlay kind="paused" partnerName="b" onResume={onResume} onBeginAnother={() => {}} />));
    fireEvent.click(getByText(/resume|Tap to resume/i));
    expect(onResume).toHaveBeenCalled();
  });
  it('ended overlay offers begin-another', () => {
    const onBeginAnother = vi.fn();
    const { getByText } = render(wrap(<StateOverlay kind="ended" partnerName="b" onResume={() => {}} onBeginAnother={onBeginAnother} />));
  ...
  it('partner-away overlay shows the partner name + copy', () => {
    const { getByText } = render(wrap(<StateOverlay kind="partnerAway" partnerName="아버님" onResume={() => {}} onBeginAnother={() => {}} />));
